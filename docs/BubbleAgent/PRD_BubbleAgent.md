...
1. 用户在右侧泡泡区任意空白位置双击，输入项目想法并选择开发深度。
...
7. 未完成 Bubble 位于画布底部；完成后浮到顶部，尺寸对应开发深度。
...
- Bubble 的双击创建、列表、打开、重命名和删除；
- 左侧空间、文件、功能列表和右侧大泡泡区；
- 深度映射 Bubble 尺寸，未完成位于底部，完成后状态驱动上浮；
- 想法输入与三档开发深度选择；
...
| 前端 | Next.js 16 + React 19 + TypeScript | App Router 明确服务端与客户端边界；静态导出继续兼容 Tauri；岗位匹配度高 |
| UI | Motion + Phosphor Icons + 原生 CSS 设计系统 | 用状态驱动 Bubble 位移；统一图标、主题、响应式和可访问性，不依赖重型组件库 |
...
| 模型接入 | 自定义 `StructuredModel` 协议 + httpx OpenAI-compatible 客户端 | Demo 与真实模型共用结构化接口，业务节点不耦合供应商 SDK |
...
| 凭据 | 进程环境变量 | MVP 不把 API Key 写入数据库、日志和 Git；系统凭据库列为后续增强 |
...
- 后端仅绑定 `127.0.0.1:8765`；随机端口协商列为后续增强；
...
│     ├─ src/                  # 泡泡画布、API 与类型
│     └─ src-tauri/            # Tauri 壳与 Rust sidecar 管理
├─ backend/
│  ├─ app/
│  │  ├─ api/                  # FastAPI routers
